...
# Mapping of XML field names → tenor in months
NOMINAL_FIELDS = {
    "d:BC_1MONTH": 1,
    "d:BC_2MONTH": 2,
    "d:BC_3MONTH": 3,
...
# OData namespace used by Treasury XML
NS = {
...
def _fetch_xml(data_type: str, year: int) -> str:
    """Fetch raw XML from Treasury.gov for a given year."""
...
def _parse_yield_entries(xml_text: str, field_map: dict, curve_type: str) -> list[dict]:
    """Parse Treasury XML into a list of row dicts."""
    ...
    for entry in root.findall(".//a:entry", NS):
        ...
        # Date format: "2026-02-19T00:00:00"
        try:
...
def fetch_ust_nominal(year: int | None = None) -> pd.DataFrame:
    """
    Fetch US Treasury nominal yield curve for a given year.
    Returns DataFrame with columns: fecha, tenor_months, yield_value, curve_type
    """
    if year is None:
...
def fetch_ust_tips(year: int | None = None) -> pd.DataFrame:
    """
    Fetch US Treasury TIPS (real) yield curve for a given year.
    Returns DataFrame with columns: fecha, tenor_months, yield_value, curve_type
    """
    if year is None:
        year = date.today().year
...
def fetch_all_curves(year: int | None = None) -> pd.DataFrame:
    """Fetch both nominal and TIPS curves, returns combined DataFrame."""
