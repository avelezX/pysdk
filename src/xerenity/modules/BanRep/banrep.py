...
class BanRep_Functions:
    ...
    def get_econ_data_last(self,id_serie: int) -> APIResponse:
        return self.xty.session.table(table_name="banrep_serie_value").select("*").eq(column="id_serie", value=id_serie).order(column="fecha", desc=True).limit(1).execute()
    
    def get_econ_data_last_n(self,id_serie: int,n:int) -> APIResponse:
        return self.xty.session.table(table_name="banrep_serie_value").select("*").eq(column="id_serie", value=id_serie).order(column="fecha", desc=True).limit(n).execute()