...
class full_ibr_curve_creation:
    def __init__(self, desired_date_valuation=ql.Date.todaysDate(), calendar=calendar_colombia(), day_to_avoid_fwd_ois=7, db_info=None):
        ...
        if is_past_noon():
            ...
            init_date = ql_to_datetime(
                calendar.advance(self.desired_date, -1, ql.Days))
    ...
    def crear_curva(self, days_to_on=1):
        ...
        ibr_cluster_mean = ibr_mean_query(
            ibr_data, self.init_date, self.final_date, day_to_avoid_fwd_swaps=self.day_to_avoid_fwd_ois)
        ...
        OIS_helpers.append(depo_helpers_ibr(
            self.db_info['ibr_1m'], days_to_on, ql.Months))
